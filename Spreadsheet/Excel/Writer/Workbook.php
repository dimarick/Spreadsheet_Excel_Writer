...
class Spreadsheet_Excel_Writer_Workbook extends Spreadsheet_Excel_Writer_BIFFwriter
{
    ...
    public function addWorksheet($name = '')
    {
        ...
        if ($this->_BIFF_version != 0x0600) {
            if (strlen($name) > 31) {
                throw new \OverflowException("Sheetname $name must be <= 31 chars");
            }
        } else {
        ...
        for ($i = 0; $i < $total_worksheets; $i++) {
            if ($this->_worksheets[$i]->getName() == $name) {
                throw new \LogicException("Worksheet '$name' already exists");
            }
        }
    ...
    public function setCustomColor($index, $red, $green, $blue)
    {
        ...
        if ($index < 8 or $index > 64) {
            throw new \InvalidArgumentException("Color index $index outside range: 8 <= index <= 64");
        }
        ...
        if (($red < 0 or $red > 255) ||
            ...
            ($blue < 0 or $blue > 255)) {
            throw new \InvalidArgumentException("Color component outside range: 0 <= color <= 255");
        }
