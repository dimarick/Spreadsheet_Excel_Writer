...
class Spreadsheet_Excel_Writer_Worksheet extends Spreadsheet_Excel_Writer_BIFFwriter
{
        ...
        foreach ($this->_colinfo as $key => $colinfo) {
            ...
            if ($firstcol > $existing_start && $firstcol < $existing_end) { // trim the existing range to the beginning of the new range
                ...
                if ($lastcol < $existing_end) { // split the existing range by adding a range after our new range
                    $this->_colinfo[] = array($lastcol + 1, $existing_end, $colinfo[2], /* format */
    ...
    protected function _processBitmap($bitmap)
    {
        ...
        if (!$bmp_fd) {
            throw new \RuntimeException("Couldn't import $bitmap");
        }
        ...
        if (strlen($data) <= 0x36) {
            throw new \RuntimeException("$bitmap doesn't contain enough data.\n");
        }

        // The first 2 bytes are used to identify the bitmap.
        $identity = unpack("A2ident", $data);
        if ($identity['ident'] !== "BM") {
            throw new \RuntimeException("$bitmap doesn't appear to be a valid bitmap image.\n");
        }
        ...
        $data = substr($data, 8);
        if ($width > 0xFFFF) {
            throw new \RuntimeException("$bitmap: largest image width supported is 65k.\n");
        }
        if ($height > 0xFFFF) {
            throw new \RuntimeException("$bitmap: largest image height supported is 65k.\n");
        }
        ...
        if ($planes_and_bitcount[2] != 24) { // Bitcount
            throw new \RuntimeException("$bitmap isn't a 24bit true color bitmap.\n");
        }
        if ($planes_and_bitcount[1] != 1) {
            throw new \RuntimeException("$bitmap: only 1 plane supported in bitmap image.\n");
        }
        ...
        if ($compression['comp'] != 0) {
            throw new \RuntimeException("$bitmap: compression not supported in bitmap image.\n");
        }
